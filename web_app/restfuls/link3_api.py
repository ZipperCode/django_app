...
from web_app.model.link import AccountLink3
...
def page_list(request: HttpRequest):
    ...
    try:
        res, count = link_service.query_list_page_new(AccountLink3.objects, body, start_row, end_row)
    except:
        logging.info("link3PageList# err = %s", traceback.format_exc())
        res = []
        count = 0
...
def add_data(request: HttpRequest):
    ...
    try:
        link_service.insert_new(AccountLink3.objects, body, user)
...
def update_data(request: HttpRequest):
    ...
    if user.role == USER_ROLE_UPLOADER:
        return RestResponse.failure("非业务员不可操作")
    try:
        link_service.update_new(AccountLink3.objects, body, user)
    except BusinessException as e:
        return RestResponse.failure(e.errmsg)
...
def delete_data(request: HttpRequest):
    ...
    link_service.delete_link(AccountLink3.objects, body)
...
def export(request: HttpRequest):
    body = utils.request_body(request)
    query_list = AccountLink3.objects.values(
    ...
    logging.info("link3_export id # 数据大小 = %s, count = %s", data_size, count)
    file_path = None
    for index in range(0, count):
        ...
        logging.info("link3_export# index = %s 转换ExcelBean成功 = %s", index, len(excel_list))
        if len(excel_list) <= 0:
            continue
        if not file_path:
            ...
            if not os.path.exists(temp_path):
                os.mkdir(temp_path)
            logging.info("link3_export#开始生成excel文件，目录 = %s", temp_path)
            file_path = create_excel(excel_list, temp_path)
            if not file_path:
                return HttpResponse(status=404, content="下载失败，创建excel失败")
        else:
            logging.info("link3_export# index = %s追加生成excel文件，文件 = %s", index, file_path)
            append_excel(excel_list, file_path)

    logging.info("link3_export#数据填充完毕，最终文件 = %s", file_path)
