...
def search_list(request: HttpRequest):
    ...
    id_no = str(query_params.get('id'))
    if utils.str_is_null(str(account_id)) and utils.str_is_null(id_no):
        return RestResponse.success("")
    ...
    if not utils.str_is_null(id_no) and utils.is_int(id_no):
        query_args['id'] = int(id_no)

    if not utils.str_is_null(account_id):
        query_args['account_id__contains'] = account_id
    ...
    def query_data(_query, t):
        _queryset = _query.filter(**query_args).values(
            'id', 'account_id', 'is_bind', 'used', 'create_time'
        )
    ...
    query_data(WaAccountId4.objects.filter(), "WhatsApp4")
    query_data(WaAccountId5.objects.filter(), "WhatsApp5")
    query_data(WaAccountId6.objects.filter(), "WhatsApp6")
