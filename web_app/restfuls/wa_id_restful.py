    ...
    with transaction.atomic():
        if not utils.str_is_null(used):
            ...
            if is_admin:
                ...
                _q = WaUserIdRecord.objects.filter(account_id=account_id)
                if _q.exists():
                ...
                _q = WaUserIdRecord.objects.filter(user_id=user_id, account_id=account_id)
                if _q.exists():
                    _q.update(used=_status, update_time=time_utils.get_now_bj_time_str())
...
def account_id_upload(request: HttpRequest):
    ...
    logging.info("account_wa_id_upload#a_id = %s", a_id)
    if utils.str_is_null(a_id):
        return RestResponse.failure("上传失败，id不能为空")

    user_id = request.session.get('user_id')
    if not http_utils.check_user_id(user_id):
        logging.info("account_wa_id_upload#上传失败，未获取到用户信息")
    ...
    if not created:
        logging.info("account_wa_id_upload#已经存在")
...
def handle_used_state(request: HttpRequest):
    body = utils.request_body(request)
    logging.info("批量修改使用状态#wa_id#body = %s", str(body))
    try:
        ids = body.get("ids", "")
        ids = ids.split(",")
        used = body.get('used')
        used = utils.get_status(used)
        WaAccountId.objects.filter(id__in=ids).update(used=used)
        WaUserIdRecord.objects.filter(account__id__in=ids).update(used=used)
