...
class WaAccountId4(models.Model):
...
class WaAccountQr4(models.Model):
    id = models.AutoField(primary_key=True)
...
class WaUserIdRecord4(models.Model):
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(to="User", verbose_name="用户外键", null=True, on_delete=models.SET_NULL)
    account = models.ForeignKey(to="WaAccountId4", verbose_name="账号ID外键", null=True, on_delete=models.SET_NULL)
    used = models.SmallIntegerField("是否使用", default=UsedStatus.Default, choices=UsedStatus.choices)
...
class WaUserQrRecord4(models.Model):
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(to="User", verbose_name="用户外键", null=True, on_delete=models.SET_NULL)
    account = models.ForeignKey(to="WaAccountQr4", verbose_name="二维码表外键", null=True, on_delete=models.SET_NULL)
    used = models.SmallIntegerField("是否使用", default=UsedStatus.Default, choices=UsedStatus.choices)
    create_time = models.DateTimeField("创建时间", null=True, auto_now_add=True)
    update_time = models.DateTimeField("更新时间", null=True, auto_now=True)
