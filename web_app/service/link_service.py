...
def query_list_page_new(queryset: QuerySet, body, start_row, end_row):
    logging.info("query_link_list_page# body = %s", body)
    date_start = body.get('date_start')
    date_end = body.get('date_end')
    op_username = body.get('op_username')
...
def insert_new(queryset: QuerySet, body, user):
    link = body.get("link")
    remark = body.get("remark")
...
def update_new(queryset: QuerySet, body, user):
    link_id = body.get("id")
    link = body.get("link")
    remark = body.get("remark")
    exists = AccountLink.objects.filter(id=link_id).exists()
    ...
    queryset.filter(id=link_id).update(
        link=link,
        remark=remark,
        update_time=time_utils.get_now_utc()
    )
...
def delete_link(queryset: QuerySet, body):
    link_id = body.get("id")
    queryset.filter(id=link_id).delete()
